...
func (p *Postgres) CreateOrEditWorkingHours(workhours *models.WorkingHours) (*models.WorkingHours, error) {
	existingWH, _ := p.GetWorkingHours(workhours.DoctorID)
...
func (p *Postgres) GetWorkingHours(id_doctor int) (*models.WorkingHours, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			fmt.Println("no working hours found for doctor_id: %d", id_doctor)
			return nil, fmt.Errorf("failed to GetWorkingHours: %w", err)
		}
		fmt.Println("failed to query working hours")
		return nil, fmt.Errorf("failed to GetWorkingHours: %w", err)
	}
